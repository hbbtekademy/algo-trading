...
window_end = '2022-01-22 00:00:00'
rsi_15min = 30
rsi_60min = 50
...
for index, row in df.iterrows():
    ...
    prev_high = prev_candle.iloc[0]['High']
    if((prev_high - row['Low'])*lot_size > stop_loss):
        # print(index, 'Stop Loss greater than 6000 INR. Do not trade',              (prev_high - row['Low'])*lot_size)
        continue
    else:
        # print(index, 'Stop Loss within range',              (prev_high - row['Low'])*lot_size)
        temp_df.loc[index] = row
...
for index, row in df.iterrows():
    ...
    sig_stop_loss = stop_loss_candle.iloc[0]['High']
    sig_entry = row['Low']
    sig_daily_mov_pct = np.round(helpers.get_sell_tanaji_pct(
...
for signal in signals:
    helpers.generate_sell_signal_pnl(signal, df_15min, stop_gain=stop_gain)
